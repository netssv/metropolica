...
export function drawBuildingsLayer(
  ...
  for (const { tile, col, row, projected } of drawOrder) {
    ...
    if (isBuilding && isTileInViewport(projected, canvasWidth, canvasHeight, zoom, margin)) {
      visibleTiles++;
    ...
    // Interleaved per-tile vehicle rendering in strict isometric depth order
    if (transit?.drawVehiclesForTile) {
      transit.drawVehiclesForTile(
        ctx, col, row, ox, oy, zoom, time, simState?.speed ?? 1,
        (simState?.citizens ? Object.values(simState.citizens).flat() : []) as any[],
