...
export const BUSINESS_ACCENT_MIN_DISTANCE = 6;
...
export function selectBusinessMarkers(tiles: AccentTile[], service: BusinessAccentType): AccentTile[] {
  ...
  const candidates = tiles.filter(t => t.type === building && (((t.col * 31 + t.row * 17) >>> 0) % CANDIDATE_PERIOD === 0)).sort((a, b) => a.row - b.row || a.col - b.col);
  for (const tile of candidates) if (accepted.every(other => Math.abs(tile.col - other.col) + Math.abs(tile.row - other.row) >= BUSINESS_ACCENT_MIN_DISTANCE)) accepted.push(tile);
  return accepted;
}
