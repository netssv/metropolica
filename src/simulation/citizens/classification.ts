...
export function workplaceFor(citizen: Pick<Citizen, 'occupation'>): { zone: string; label: string; specialty?: 'hospital' | 'mall-government' | 'bank' } {
  ...
  if (/bancario|banquero|banco/.test(occupation)) return { zone: 'bldg-c', label: 'banco / finanzas', specialty: 'bank' };
