...
export const handleCommands: Handler = async (req, res, context) => {
  ...
  if (req.method !== 'POST' || !['/api/advance', '/api/speed', '/api/command', '/api/inspect'].includes(path ?? '')) return false;
  try {
    const body = JSON.parse(await readBody(req) || '{}');
    if (path === '/api/advance') { context.game.advance(body.days || 1); json(res, 200, { success: true, day: context.game.clock.currentDay }); return true; }
    if (path === '/api/speed') { context.game.clock.setSpeed(body.speed); json(res, 200, { success: true, speed: context.game.clock.currentSpeed }); return true; }
    if (path === '/api/command') {
